...
import { Button, Upload, message, Modal, Space, Typography } from 'antd';
...
export const DataBackup: React.FC<DataBackupProps> = ({ onDataChange }) => {
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      message.error(`数据导入失败: ${error instanceof Error ? error.message : '未知错误'}`);
    } finally {
      setImporting(false);
  ...
  return (
    <div style={{ 
      ...
      <div style={{ marginBottom: 16 }}>
        <Text strong style={{ fontSize: 16 }}>数据备份</Text>
